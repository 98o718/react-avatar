...
export default function createAvatarComponent({ sources = [] }) {
  ...
  const sourcePropTypes = sources.reduce(
    (r, s) => Object.assign(r, s.propTypes),
    {}
  )
  ...
  class Avatar extends PureComponent {
    ...
    _scaleTextNode = (node, retryTTL = 16) => {
      ...
      if (!node || unstyled || this.state.src || !this.mounted) return
    ...
    _renderAsText() {
      ...
      // the correct `ref`.
      const key = [this.state.value, this.props.size].join('')

      return (
        <div className={className + ' sb-avatar__text'} style={initialsStyle}>
